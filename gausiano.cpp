...
int main()
{
    ...
    GaussianFilter g;
    int width, height;
    double **image = stb.imread("/home/mickel/Downloads/examen-paralela-master/imgPruebas/500x500.png", width, height); // Lectura de imagen
    double sigma = 1;                                                                                                // Valor ajustable
    image = g.gaussianFilter(image, width, height, sigma);                                                           // Aplica filtro gaussiano
    // Guardar imagen
    stb.imwrite("imageGasParalela.jpg", image, width, height);
    // Liberar puntero
